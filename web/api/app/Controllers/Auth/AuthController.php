...
################################################
	...
	public function postChangePassword($req,$res)
	{
		...
		{
			$policy = APIPWPolicy::select()->first(1)->toArray();
		} else {
			$policy = [];
		}
		// Set default values for policy keys if they are not set
		$policy = array_merge([
			'api_pw_length' => 8,
			'api_pw_uppercase' => false,
			'api_pw_lowercase' => false,
			'api_pw_special' => false,
			'api_pw_same' => true,
			'api_pw_numbers' => false
		], $policy);
		$validation = $this->validator->validate($req, [
