...
########	Authentication Datatables ###############START###########
	...
	public function postAuthenticationDatatables($req,$res)
	{
		...
		// FIX FOR PHP8.3
		// Check if $queries is not empty and contains the expected index
...
########	File Tree ###############START###########
	...
	public function postLogDelete($req,$res)
	{
		...
		if ($allParams['period'] == 'all') {
			$period = 'all';
		} elseif (preg_match('/^[0-9]{1,3}\s(year[s]{0,1}|month[s]{0,1})/', $allParams['period'])) {
			$dateCount = new \DateTime;
			$period = $dateCount->modify('-'.$allParams['period']);
			$period = $dateCount->format('Y-m-d H:i:s');
		}
