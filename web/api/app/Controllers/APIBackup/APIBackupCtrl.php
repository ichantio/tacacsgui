...
private $listOfTacacsReportsTables = '--tables tac_log_accounting tac_log_authorization tac_log_authentication';
	...
	public function postBackupDatatables($req,$res)
	{
		...
		// Example payload {"page":1,"pageSize":5,"searchTerm":"","filterTerm":[],"sortColumn":"filename","sortDirection":"asc","extra":{"type":"full"}}
		$params=$req->getParams(); //Get ALL parameters form Datatables

		$data['draw'] = isset($params['draw']) ? intval($params['draw']) : 0;
        $backup_start = isset($params['start']) ? intval($params['start']) : 0;
        $backup_length = isset($params['length']) ? intval($params['length']) : 10;
        $backup_list_order = (isset($params['sortDirection']) && $params['sortDirection'] === 'asc') ? 'asc' : 'desc';
        $backup_type = (isset($params['extra']) && isset($params['extra']['type'])) ? $params['extra']['type'] : 'full';
		$data['recordsTotal'] = $data['recordsFiltered'] = 1;

        $START = $backup_start + 1;
        $LENGTH = $backup_length + $backup_start;
		$ORDER = $backup_list_order;
        $TYPE = $backup_type;
        $data['test222'] = $backup_type;
	...
	public function postBackupMake($req,$res)
	{
		...
		switch ($allParams['type']) {
			...
			case 'full':
				...
				break;
			case 'tcfg':
					$data['result'] = $this->makeBackup(['make'=>'tcfg', 'diff' => $allParams['diff']]);
				break;
			default:
				$data['error']['status']=true;
				$data['error']['message']='Type backup error';
