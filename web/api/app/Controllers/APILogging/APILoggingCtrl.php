...
class APILoggingCtrl extends Controller
{
	...
	public function postLoggingDelete($req,$res)
	{
		...
		$period = '';
		// FIX FOR PHP8.3
		if ($allParams['period'] == 'all') {
			$period = 'all';
		} elseif (!preg_match('/^[0-9]\s(year[s]{0,1}|month[s]{0,1})', $allParams['period']))
		{
		...
		$data['period'] = $period;
		// Create an instance of APILogging and call delete method
		if ($period == 'all') {
			$data['result'] = APILogging::query()->delete();
		} else {
			$data['result'] = APILogging::where('created_at', '<=', $period)->delete();
		}
