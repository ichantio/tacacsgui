...
################################################
	...
	private function startCron($params, $read=false){
		$cron=[];
		if (!$read) {
			$cmPeriod = isset($params['cm']['period']) ? $params['cm']['period'] : 'day';
			$cmTime = isset($params['cm']['time']) ? $params['cm']['time'] : '00:00';
			$cmWeek = isset($params['cm']['week']) ? $params['cm']['week'] : 0;
			$gitPeriod = isset($params['git']['period']) ? $params['git']['period'] : 60;
			...
			$yaml = Yaml::dump( $cron, 4 );
			$filePath = '/opt/tgui_data/confManager/cron.yaml';
			if (file_put_contents($filePath, $yaml) == false) {
				throw new \Exception('Failed to write YAML file');
			}
			$resultCronStart = CMDRun::init()
				->setCmd(MAINSCRIPT)
				->setAttr([
					'run',
					'cmd',
				...
				->get();
			if ($resultCronStart != 'done' ) {
				throw new \Exception('Failed to start cron');
			}
			else return $resultCronStart;
		}
		$resultCronStart = CMDRun::init()
			->setCmd(MAINSCRIPT)
			->setAttr([
