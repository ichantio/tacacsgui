...
class TACUsersCtrl extends Controller
{
	...
	public function itemValidation($req = [], $state = 'add'){
		...
		foreach ($expectedKeys as $key => $defaultValue) {
		...
		return $this->validator->validate($req, [
			'username' => v::noWhitespace()
				->notEmpty()
				->theSameNameUsed( '\tgui\Models\TACUsers', $id, 'username' )
				->not( v::oneOf(
					v::contains('@'),
					v::contains('='),
					v::contains('*'),
			...
			'login' => v::when( v::oneOf(v::checkLoginType($req['login_flag']), v::nullType()), v::alwaysValid(), v::noWhitespace()->
					...
					length($policy['tac_pw_length'], 64)->
					notEmpty()->
					passwdPolicyUppercase($policy['tac_pw_uppercase'], $req['login_flag'])->
					passwdPolicyLowercase($policy['tac_pw_lowercase'], $req['login_flag'])->
					passwdPolicySpecial($policy['tac_pw_special'], $req['login_flag'])->
					passwdPolicyNumbers($policy['tac_pw_numbers'], $req['login_flag'])->setName('Login Password') ),
			...
			'email' => v::optional(v::stringType()->email()->setName('Email')),
			'valid_from' => v::optional(v::stringType()->date('Y-m-d')->setName('Valid From')),
			'valid_until' => v::optional(v::stringType()->date('Y-m-d')->setName('Valid Until'))
		]);
...
################################################
	...
	public function postUserAdd($req,$res)
	{
		...
		// FIX for PHP8.3
		$groups_bind = [];
		// Check if $groups is not empty and is an array
		if (!empty($groups) && is_array($groups)) {
			for ($i = 0; $i < count($groups); $i++) {
				// Ensure that group_id exists and is not empty
				if (!empty($groups[$i])) {
		...
		// Insert data only if there are valid groups to insert
		if (!empty($groups_bind)) {
