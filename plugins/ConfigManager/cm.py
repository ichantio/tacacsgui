...
if args.cron_set:
	...
	# Convert git period to commit
	git_commit_period = git.get('period', 60)
	if git_commit_period == 60:
		git_cron_schedule = '0 * * * *'
	else:
		git_cron_schedule = f'*/{git_commit_period} * * * *'
	# Create cron content
	cron_content = f'''#### TGUI CM ####
{re.sub('^0', '', minutes)} {re.sub('^0', '', hours)} * * {week} {os.path.realpath(__file__)} -c /opt/tgui_data/confManager/config.yaml > /dev/null 2>/dev/null &
#### TGUI GIT ####
{git_cron_schedule} {os.path.realpath(__file__)} --git-commit -c /opt/tgui_data/confManager/config.yaml > /dev/null 2>/dev/null &
'''
	try:
...
for query in list(data_loaded['queries']):
	...
	if not args.test_queries and os.path.exists(data_loaded['git']['path']+path+query['name']):
		...
		if not os.path.exists(os.path.dirname(data_loaded['git']['path']+path+query['name'])):
			try:
				...
				if args.debug: deb.cm_debug.show( message = "Can't create directory: "+path)
				dir_ok = False
		if dir_ok:
	...
	try:
		...
		if args.test_queries:
			...
			config_file_path = os.path.join(data_loaded['git']['path'] + path, query['name'])
			if os.path.exists(config_file_path):
				try:
					os.chmod(config_file_path, 0o640)
					if args.debug:
						deb.cm_debug.show( message = "File permissions set to 640")
				except Exception as e:
					if args.debug:
						deb.cm_debug.show( message = "Error setting file permissions: {}".format(e) )
			else:
				if args.debug:
					deb.cm_debug.show( message = "File not found: {}".format(config_file_path) )					
